...
MetalBondedUtilities::MetalBondedUtilities(MetalContext& context) : context(context), maxBonds(0), allGroups(0), hasInitializedKernels(false) {
}

void MetalBondedUtilities::addInteraction(const vector<vector<int> >& atoms, const string& source, int group) {
    if (atoms.size() > 0) {
...
string MetalBondedUtilities::addArgument(cl::Memory& data, const string& type) {
    arguments.push_back(&data);
    argTypes.push_back(type);
    return "customArg"+context.intToString(arguments.size());
}

string MetalBondedUtilities::addArgument(ArrayInterface& data, const string& type) {
    return addArgument(context.unwrap(data).getDeviceBuffer(), type);
}

string MetalBondedUtilities::addEnergyParameterDerivative(const string& param) {
...
void MetalBondedUtilities::addPrefixCode(const string& source) {
...
void MetalBondedUtilities::initialize(const System& system) {
...
string MetalBondedUtilities::createForceSource(int forceIndex, int numBonds, int numAtoms, int group, const string& computeForce) {
...
void MetalBondedUtilities::computeInteractions(int groups) {
