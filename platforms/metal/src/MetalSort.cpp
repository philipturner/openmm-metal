...
MetalSort::MetalSort(MetalContext& context, SortTrait* trait, unsigned int length, bool uniform) :
    ...
    cl::Program program = context.createProgram(context.replaceStrings(MetalKernelSources::sort, replacements));
    ...
    if (vendor.size() >= 6 && vendor.substr(0, 6) == "NVIDIA") {
        ...
        useShortList2 = (dataLength <= MetalContext::ThreadBlockSize*context.getNumThreadBlocks());
    }
    else {
