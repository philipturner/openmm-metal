...
MetalArray::MetalArray(MetalContext& context, size_t size, int elementSize, const std::string& name, cl_int flags) : buffer(NULL) {
    initialize(context, size, elementSize, name, flags);
}

MetalArray::MetalArray(MetalContext& context, cl::Buffer* buffer, size_t size, int elementSize, const std::string& name) : buffer(NULL) {
...
void MetalArray::initialize(ComputeContext& context, size_t size, int elementSize, const std::string& name) {
    initialize(dynamic_cast<MetalContext&>(context), size, elementSize, name, CL_MEM_READ_WRITE);
}

void MetalArray::initialize(MetalContext& context, size_t size, int elementSize, const std::string& name, cl_int flags) {
    if (buffer != NULL)
...
void MetalArray::initialize(MetalContext& context, cl::Buffer* buffer, size_t size, int elementSize, const std::string& name) {
    if (this->buffer != NULL)
        throw OpenMMException("MetalArray has already been initialized");
    this->context = &context;
    this->buffer = buffer;
...
void MetalArray::copyTo(ArrayInterface& dest) const {
    ...
    MetalArray& clDest = context->unwrap(dest);
    try {
