...
static void setPeriodicBoxSizeArg(MetalContext& cl, cl::Kernel& kernel, int index) {
...
static void setPeriodicBoxArgs(MetalContext& cl, cl::Kernel& kernel, int index) {
    if (cl.getUseDoublePrecision()) {
        kernel.setArg<mm_double4>(index++, cl.getPeriodicBoxSizeDouble());
...
public:
    PmeIO(MetalContext& cl, cl::Kernel addForcesKernel) : cl(cl), addForcesKernel(addForcesKernel) {
...
public:
    SyncQueuePreComputation(MetalContext& cl, cl::CommandQueue queue, int forceGroup) : cl(cl), queue(queue), forceGroup(forceGroup) {
    }
    void computeForceAndEnergy(bool includeForces, bool includeEnergy, int groups) {
...
public:
    SyncQueuePostComputation(MetalContext& cl, cl::Event& event, MetalArray& pmeEnergyBuffer, int forceGroup) : cl(cl), event(event),
...
void MetalCalcNonbondedForceKernel::initialize(const System& system, const NonbondedForce& force) {
    ...
    if (nonbondedMethod == Ewald) {
        ...
        if (cl.getContextIndex() == 0) {
            ...
            cl::Program program = cl.createProgram(MetalKernelSources::ewald, replacements);
        ...
        if (cl.getContextIndex() == 0) {
            ...
            if (cl.getPlatformData().useCpuPme && !doLJPME && usePosqCharges) {
                ...
                try {
                    ...
                    cl::Program program = cl.createProgram(MetalKernelSources::pme, pmeDefines);
    ...
    if (hasCoulomb && !usePosqCharges)
        cl.getNonbondedUtilities().addParameter(MetalNonbondedUtilities::ParameterInfo(prefix+"charge", "real", 1, charges.getElementSize(), charges.getDeviceBuffer()));
    ...
    if (hasLJ) {
        ...
        cl.getNonbondedUtilities().addParameter(MetalNonbondedUtilities::ParameterInfo(prefix+"sigmaEpsilon", "float", 2, sizeof(cl_float2), sigmaEpsilon.getDeviceBuffer()));
    }
    ...
    cl::Program program = cl.createProgram(MetalKernelSources::nonbondedParameters, paramsDefines);
...
double MetalCalcNonbondedForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy, bool includeDirect, bool includeReciprocal) {
    ...
    if (!hasInitializedKernel) {
        ...
        if (pmeGrid1.isInitialized()) {
            ...
            cl::Program program = cl.createProgram(cl.replaceStrings(MetalKernelSources::pme, replacements), pmeDefines);
            ...
            if (doLJPME) {
                ...
                program = cl.createProgram(MetalKernelSources::pme, pmeDefines);
